export class Grid {
    ...
    neighbours(row, col) {
        ...
        if (row === 0 && col === 0) {
            ...
            let neighbours = [this.south(row, col), this.east(row, col), this.west(row, col)];
            return neighbours;
        } else if (row === this.rows.length) {
            let neighbours = [this.north(row, col), this.east(row, col), this.west(row, col)];
            return neighbours;
        } else if (col === 0) {
            let neighbours = [this.north(row, col), this.south(row, col), this.east(row, col)];
            return neighbours;
        } else if (col === this.cols.length) {
            let neighbours = [this.north(row, col), this.south(row, col), this.west(row, col)];
            return neighbours;
        } else {
            let neighbours = [];
