...
function visitedCell(grid, start, goal) {
    ...
    while (!rute.isEmpty()) {
        ...
        if (!currentCell.isVisited) {
            ...
            if (!currentCell.north && grid.north(row, col) && !grid.north(row, col).isVisited) {
                rute.push({ row: row - 1, col: col }); // row - 1 = north
                moved = true;
            } else if (!currentCell.east && grid.east(row, col) && !grid.east(row, col).isVisited) {
                rute.push({ row: row, col: col + 1 }); // col + 1 = east
                moved = true;
            } else if (!currentCell.south && grid.south(row, col) && !grid.south(row, col).isVisited) {
                rute.push({ row: row + 1, col: col }); // row + 1 = south
                moved = true;
            } else if (!currentCell.west && grid.west(row, col) && !grid.west(row, col).isVisited) {
